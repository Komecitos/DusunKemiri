...
<div class="max-w-3xl mx-auto bg-white shadow rounded-2xl p-6 font-roboto border border-sogan/30">
    ...
    <form action="{{ route('admin.jadwal.store') }}" method="POST" class="space-y-4 text-sm">
        ...
        <div>
            <label class="block font-semibold text-sogan mb-1">Judul Kegiatan</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                <label class="block font-semibold text-sogan mb-1">Lokasi</label>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block font-semibold text-sogan mb-1">Waktu Mulai</label>
                <input type="time" name="waktu_mulai" value="{{ old('waktu_mulai') }}"
                    class="w-full border border-kunyit rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-kunyit" required>
            </div>

            <div>
                <label class="block font-semibold text-sogan mb-1">Waktu Selesai</label>
        ...
        <div class="flex justify-between mt-6">
            ...
            <button type="submit"
                class="bg-sogan hover:bg-kunyit hover:text-sogan text-white px-6 py-2 rounded shadow transition">Simpan</button>
        </div>
    </form>
</div>
@endsection